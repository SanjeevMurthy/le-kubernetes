...
check_persisted "net.ipv4.ip_forward is written to a file, so forwarding survives a reboot" \
...
check "the inside peer can reach the outside network at all" \
  in_peer "$IN" curl -s --max-time 5 -o /dev/null "http://$OUTIP/"
SEEN=$(grep -E '"GET ' "$STATE/out.log" 2>/dev/null | tail -1 | awk '{print $1}')
check_eq "the outside server sees this host's address, so the source was masqueraded" \
  "$OUTHOST" "${SEEN:-nothing was logged}"
