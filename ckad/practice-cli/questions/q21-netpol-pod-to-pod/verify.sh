...
echo "Checking a NetworkPolicy exists in backend namespace..."
NETPOL_COUNT=$(kubectl get networkpolicy -n backend --no-headers 2>/dev/null | wc -l | tr -d ' ')
if [[ "$NETPOL_COUNT" -ge 1 ]]; then
  echo "  PASS: NetworkPolicy found in backend"
  ((PASS++))
else
  echo "  FAIL: No NetworkPolicy found in backend namespace"
  ((FAIL++))
fi

# Get the first NetworkPolicy name
NETPOL_NAME=$(kubectl get networkpolicy -n backend --no-headers -o custom-columns=':metadata.name' 2>/dev/null | head -1 | tr -d ' ')

echo "Checking NetworkPolicy targets pods with app=database..."
POD_SEL=$(kubectl get networkpolicy "$NETPOL_NAME" -n backend -o jsonpath='{.spec.podSelector.matchLabels.app}' 2>/dev/null)
...
API_MATCH=$(kubectl get networkpolicy "$NETPOL_NAME" -n backend \
...
PORT_MATCH=$(kubectl get networkpolicy "$NETPOL_NAME" -n backend \
