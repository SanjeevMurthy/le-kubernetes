...
show_main_menu() {
  print_header "${ICON_TARGET} CKAD Exam Practice CLI"
  ...
  echo -e "  ${CYAN}[3]${RESET}  ${WHITE}Random incomplete question${RESET}"
  echo -e "  ${CYAN}[4]${RESET}  ${WHITE}Show progress${RESET}"
  echo -e "  ${CYAN}[5]${RESET}  ${WHITE}Exit${RESET}"
  echo ""
  echo -ne "  ${BOLD}> ${RESET}"
}

show_question_list() {
  print_header "${ICON_BOOK} All CKAD Questions"
...
show_question_selector() {
  echo ""
  echo -e "  ${BOLD}${WHITE}Enter question number (1-24) or 'b' to go back:${RESET}"
