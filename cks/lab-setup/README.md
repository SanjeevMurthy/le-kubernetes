...
`./cks --env` prints the authoritative answer for whatever host you are on, by matching each question's `needs` tags against the environment. As a rule:

| Tier | Covers |
|---|---|
| minikube with Calico (tier 1) | NetworkPolicy, RBAC, ServiceAccounts, Pod Security Admission, securityContext and immutability, Trivy and static analysis, Ingress TLS, Kyverno and Gatekeeper, CSRs, Dockerfile review |
| Killercoda or KodeKloud (tier 2) | AppArmor, seccomp, Falco, kube-bench and CIS, API server and kubelet flags, audit logging, encryption at rest and etcdctl, gVisor, kubeadm upgrade, strace, binary checksums, host hardening |
| Neither without extra setup | Cilium L7 policy and transparent encryption (needs a Cilium cluster), Istio mTLS (needs Istio). Killercoda has a Cilium playground; Istio has no free scenario, so read the recipe and make cards. |
